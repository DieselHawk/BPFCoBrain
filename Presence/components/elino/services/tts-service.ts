...
export type TTSEngine = 'web-speech' | 'voicevox' | 'openai' | 'elevenlabs' | 'google-tts' | 'aivis-speech' | 'style-bert-vits2' | 'openai-compat-tts' | 'fish-audio' | 'kokoro' | 'none';
...
class TTSService {
  ...
  private config: TTSConfig = {
    ...
    fishAudio: { apiKey: '', voiceId: '', speed: 1.0, emotion: 'neutral' },
  ...
  async speak(text: string): Promise<void> {
    ...
    } else if (this.config.engine === 'fish-audio') {
      await this.speakFishAudio(text);
    } else if (this.config.engine === 'kokoro') {
      await this.speakKokoro(text);
    } else {
  ...
  private async speakFishAudio(text: string): Promise<void> {
    const cfg = this.config.fishAudio;
    if (!cfg.apiKey) {
      console.warn('Fish Audio: APIキーが未設定');
      this.speakWebSpeech(text);
      return;
    }
    ...
    try {
      ...
      for (const sentence of sentences) {
        ...
        const audioData: Buffer = await platform.fishAudioSynthesize({
          text: sentence,
          apiKey: cfg.apiKey,
          voiceId: cfg.voiceId,
          speed: cfg.speed,
        ...
        this.callbacks.onStart?.();
        window.dispatchEvent(new CustomEvent(TTS_PHONEME_READY_EVENT, {
          detail: { type: 'fish-audio', text: sentence, rate: cfg.speed || 1.0 }
        }));
        await this.playCloudTTS(audioData, 'audio/mpeg', 'Fish Audio S2', sentence, cfg.speed);
      }
    } catch (err) {
      console.error('Fish Audio合成エラー:', err);
      console.warn('Web Speechにフォールバック');
      this.speakWebSpeech(text);
    }
  }

  // ====== Kokoro TTS（ローカル・完全無料） ======
  private async speakKokoro(text: string): Promise<void> {
    try {
      const audioData = await platform.kokoroSynthesize({
        text,
        voice: this.config.kokoro.voice,
      });
      await this.playCloudTTS(audioData, 'audio/wav', 'kokoro', text, 1.0);
    } catch (err) {
      console.error('Kokoro TTS合成エラー:', err);
      console.warn('Web Speechにフォールバック');
      this.speakWebSpeech(text);
    }
  }
