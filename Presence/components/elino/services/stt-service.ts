...
export interface STTConfig {
  enabled: boolean;
  engine: 'web-speech' | 'whisper' | 'whisper-local' | 'groq-whisper';
  autoSend: boolean;
  alwaysOn: boolean;
  lang: string;
  whisperModel?: string;
...
class STTService {
  ...
  async start(): Promise<boolean> {
    ...
    if (this.config.engine === 'web-speech') {
      ...
      // whisper / whisper-local / groq-whisper 共通（MediaRecorder）
    ...
    try {
      ...
      if (this.config.engine === 'whisper-local') {
        ...
        if (api?.whisperTranscribe) {
          ...
          text = await api.whisperTranscribe({ wavBuffer, model: 'tiny', lang });
        } else if (api?.sttTranscribeLocal) {
          // フォールバック: Transformers.js
          const lang = this.config.lang.split('-')[0] || 'ja';
          text = await api.sttTranscribeLocal(pcmData.buffer as ArrayBuffer, lang);
        } else {
          throw new Error('ローカルWhisper APIが利用できません');
        }
      } else if (this.config.engine === 'groq-whisper') {
        // Groq Whisper Cloud（高速・無料）
        const pcmData = await this.convertToFloat32PCM(audioBlob);
        const wavBuffer = this.pcmToWav(pcmData, 16000);
        if (!api?.groqWhisperTranscribe) {
          throw new Error('Groq Whisper APIが利用できません');
        }
        const lang = this.config.lang.split('-')[0] || 'ja';
        text = await api.groqWhisperTranscribe({ wavBuffer, lang });
      } else {
        // Whisper API
