...
function registerTtsHandlers(ipcMain, { loadConfig, loadSettings }) {
    ...
    ipcMain.handle('fish-audio:synthesize', async (event, { text, apiKey, voiceId, speed, emotion }) => {
        if (!apiKey) throw new Error('Fish Audio APIキーが設定されていません');
        ...
        try {
            ...
            const arrayBuffer = await response.arrayBuffer();
            console.log('🐟 Fish Audio S2:', text.substring(0, 30) + '...');
            return Buffer.from(arrayBuffer);
        } catch (err) {
    ...
    ipcMain.handle('kokoro:synthesize', async (event, { text, voice }) => {
        try {
            if (!_kokoroInstance) {
                console.log('🔊 Kokoro TTS: モデル読み込み中...');
                const { KokoroTTS } = await import('kokoro-js');
                _kokoroInstance = await KokoroTTS.from_pretrained('onnx-community/Kokoro-82M-ONNX', {
                    dtype: 'q8',
                    device: 'cpu',
                });
                console.log('✅ Kokoro TTS: モデル読み込み完了');
            }

            const selectedVoice = voice || 'af_heart';
            const audio = await _kokoroInstance.generate(text, { voice: selectedVoice });
            const wavBuffer = await audio.toWav();
            console.log('🔊 Kokoro TTS:', text.substring(0, 30) + '...');
    ...
    ipcMain.handle('fish-audio:voices', async (event, { apiKey }) => {
        if (!apiKey) throw new Error('Fish Audio APIキーが設定されていません');
