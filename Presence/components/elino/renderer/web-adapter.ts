...
export async function createWebAdapter(): Promise<ElectronAPI> {
  ...
  const adapter: ElectronAPI = {
    ...
    whisperTranscribe: async () => { throw new Error('whisper-cli not available on web'); },
    whisperCheck: async () => ({ available: false, models: [] }),
    ...
    fishAudioSynthesize: (p) => ipc('fish-audio:synthesize', p),
    fishAudioVoices: (p) => ipc('fish-audio:voices', p),
    kokoroSynthesize: (p) => ipc('kokoro:synthesize', p),
