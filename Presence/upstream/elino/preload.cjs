...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  whisperTranscribe: (params) => ipcRenderer.invoke('whisper:transcribe', params),
  whisperCheck: () => ipcRenderer.invoke('whisper:check'),
  whisperDownloadModel: (params) => ipcRenderer.invoke('whisper:download-model', params),
  onWhisperDownloadProgress: (callback) => ipcRenderer.on('whisper:download-progress', (_, data) => callback(data)),

  // STT (Groq Whisper Cloud)
  groqWhisperTranscribe: (params) => ipcRenderer.invoke('groq-whisper:transcribe', params),
  ...
  // Fish Audio S2
  fishAudioSynthesize: (params) => ipcRenderer.invoke('fish-audio:synthesize', params),
  fishAudioVoices: (params) => ipcRenderer.invoke('fish-audio:voices', params),
  kokoroSynthesize: (params) => ipcRenderer.invoke('kokoro:synthesize', params),
  ...
  // Gaze center: クリックで目線中心を設定
  sendGazeCenterRequest: () => ipcRenderer.invoke('gaze:request-center'),
  onGazeCenterMode: (callback) => {
    ipcRenderer.removeAllListeners('gaze-center-mode');
    ipcRenderer.on('gaze-center-mode', () => callback());
  },
  sendGazeCenterResult: (x, y) => ipcRenderer.send('gaze:center-result', x, y),
  onGazeCenterResult: (callback) => {
    ipcRenderer.removeAllListeners('gaze-center-result');
    ipcRenderer.on('gaze-center-result', (_, result) => callback(result));
  },
  startGazePolling: () => ipcRenderer.send('gaze:start-polling'),
  stopGazePolling: () => ipcRenderer.send('gaze:stop-polling'),
  onGazeCursorUpdate: (callback) => {
    ipcRenderer.removeAllListeners('gaze-cursor-update');
    ipcRenderer.on('gaze-cursor-update', (_, x, y, w, h) => callback(x, y, w, h));
  },
});
