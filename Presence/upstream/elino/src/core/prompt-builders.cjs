...
function buildPersonaLayer(profile, personality, lang = 'ja') {
    ...
    // --- New structure: corePersona + conversationPolicy + avoidances ---
    const hasNewStructure = Array.isArray(personality.corePersona) && personality.corePersona.length > 0;

    if (hasNewStructure) {
        const isEn = lang === 'en';
        const cp = isEn ? (personality.corePersonaEn || personality.corePersona) : personality.corePersona;
        const policy = isEn ? (personality.conversationPolicyEn || personality.conversationPolicy) : personality.conversationPolicy;
        const v = isEn ? (personality.voiceEn || personality.voice) : personality.voice;
        const avoid = isEn ? (personality.avoidancesEn || personality.avoidances) : personality.avoidances;
        const sitRules = isEn ? (personality.situationalRulesEn || personality.situationalRules) : personality.situationalRules;
        const examples = isEn
            ? (personality.conversationExamplesEn || personality.conversationExamples || DEFAULT_CONVERSATION_EXAMPLES_EN)
            : (personality.conversationExamples || DEFAULT_CONVERSATION_EXAMPLES);
        const id = isEn ? (personality.identityEn || personality.identity) : personality.identity;

        if (isEn) {
            let result = `You are ${callUser}'s friend. Your name is ${companionName}.`;
            if (id) result += `\nYou see yourself as "${id}".`;
            result += `\n\n# Who you are\n${cp.map(c => `- ${c}`).join('\n')}`;
            if (policy && policy.length > 0) {
                result += `\n\n# How conversations flow (this matters most)\n${policy.map(p => `- ${p}`).join('\n')}`;
            }
            if (v && v.length > 0) {
                result += `\n\n# Speech style\n${v.map(s => `- ${s}`).join('\n')}`;
            }
            ...
            if (sitRules && sitRules.length > 0) {
                result += `\n\n# Situational reactions\n${sitRules.map(s => `- ${s}`).join('\n')}`;
            }
            result += `\n\n# Conversation examples (match this tempo and vibe)\n${examples.join('\n')}`;
            return result;
        } else {
    ...
    // --- Legacy structure: traits / weaknesses / quirks / reactions ---
