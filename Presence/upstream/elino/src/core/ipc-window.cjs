...
function register(ipcMain, ctx) {
    ...
    // 目線中心設定: settingsWindow → characterWindow へブリッジ
    ipcMain.handle('gaze:request-center', () => {
        return new Promise((resolve) => {
            // dockedモードではdockedWindowを使う
            const targetWin = ctx.dockedWindow || ctx.characterWindow;
            if (!targetWin || targetWin.isDestroyed()) {
                resolve(null);
                return;
            }
            // characterWindowにクリック待ちモードを送る
            targetWin.webContents.send('gaze-center-mode');

            // 結果を一回限り受け取る
            const handler = (event, x, y) => {
                ipcMain.removeListener('gaze:center-result', handler);
                resolve({ x, y });
            };
            ipcMain.on('gaze:center-result', handler);
    ...
    let gazePollingInterval = null;
    function startGazePolling() {
        if (gazePollingInterval) return;
        gazePollingInterval = setInterval(() => {
            const targetWin = ctx.dockedWindow || ctx.characterWindow;
            ...
            try {
                targetWin.webContents.send('gaze-cursor-update', relX, relY, bounds.width, bounds.height);
    ...
    // gaze有効時にポーリング開始
    ipcMain.on('gaze:start-polling', () => startGazePolling());
    ipcMain.on('gaze:stop-polling', () => stopGazePolling());
}
