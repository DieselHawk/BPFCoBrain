...
async function initCharacter() {
  try {
    ...
    const dockedApi = (window as any).electronAPI;
    if (dockedApi?.onGazeCenterMode) {
      dockedApi.onGazeCenterMode(() => {
        gazeCenterMode = true;
    ...
    canvas.addEventListener('mousedown', (e) => {
      if (!gazeCenterMode) return;
      const x = e.clientX / window.innerWidth;
      const y = e.clientY / window.innerHeight;
      gazeCenterMode = false;
      document.body.style.cursor = '';
      canvas.style.cursor = '';
      if (dockedApi?.sendGazeCenterResult) {
        dockedApi.sendGazeCenterResult(x, y);
      }
    });

    // マウス追従（gaze）- mousemoveでrendererのsetGazeを呼ぶ
