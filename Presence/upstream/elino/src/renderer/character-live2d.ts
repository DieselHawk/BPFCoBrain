...
export class Live2DRenderer implements CharacterRenderer {
  ...
  private gazeEyeInfluence = 1.0;
  private gazeHeadInfluence = 0.6;
  private gazeBodyInfluence = 0.3;
  private gazeSmoothing = 0.15;
  private gazeTargetX = 0;
  private gazeTargetY = 0;
  private gazeEyeCurrentX = 0;
  private gazeEyeCurrentY = 0;
  private gazeHeadCurrentX = 0;
  private gazeHeadCurrentY = 0;
  private gazeBodyCurrentX = 0;
  ...
  async init(canvas: HTMLCanvasElement, settings: CharacterSettings): Promise<void> {
    ...
    this.applyGazeSettings(settings);
  ...
  private setupModelControls() {
    ...
    // パラメータ範囲をログ出力（デバッグ用）
    const gazeParams = ['ParamAngleX', 'ParamAngleY', 'ParamAngleZ', 'ParamBodyAngleX', 'ParamBodyAngleY', 'ParamEyeBallX', 'ParamEyeBallY'];
    for (const name of gazeParams) {
      const idx = coreModel.getParameterIndex(name);
      if (idx >= 0) {
        console.log(`[GAZE-PARAM] ${name}: min=${coreModel.getParameterMinimumValue(idx)} max=${coreModel.getParameterMaximumValue(idx)} default=${coreModel.getParameterDefaultValue(idx)}`);
      } else {
        console.log(`[GAZE-PARAM] ${name}: NOT FOUND`);
      }
    }

    this.model.internalModel.update = function (dt: number, now: number) {
      if (Date.now() % 3000 < 20) {
        const im2 = self.model?.internalModel as any;
        console.log(`[UPDATE] procedural=${self.proceduralEnabled} physics=${!!im2?.physics} saved=${!!self._savedPhysics}`);
      }
      if (!coreModel) {
      ...
      if (!self.proceduralEnabled) {
        // リップシンクとgazeをパラメータに書いてから、SDKのupdateに任せる
        self.applyLipSync(coreModel);
        self.applyGazeToModel(coreModel);
        // SDKデフォルト動作（まばたき・呼吸・物理+commit）
        originalUpdate.call(this, dt, now);
        return;
      }
      ...
      self.applyGazeToModel(coreModel);
      // 物理演算デバッグ
      const im = self.model?.internalModel as any;
      if (Date.now() % 3000 < 20) console.log(`[PHYSICS] exists=${!!im?.physics} saved=${!!self._savedPhysics} enabled=${self.physicsEnabled}`);
      coreModel.update?.();
    };

    // 3. expressionManager・eyeBlink は無効化（手動制御）
    // focusController は gaze 追従用に有効のまま残す（amadea.app と同方式）
  ...
  private setupTicker() {
    ...
    this.tickerCallback = (_deltaTime: number) => {
      try {
        ...
        this.updateGaze(delta); // マウス追従（procedural有無に関わらず常時）
      } catch (e) {
  ...
  /** gaze設定を外部から更新（設定変更時） */
  applyGazeSettings(settings: CharacterSettings): void {
    const g = settings.gaze;
    this.gazeEnabled      = g?.enabled      ?? true;
    this.gazeEyeInfluence  = g?.eyeInfluence  ?? 1.0;
    this.gazeHeadInfluence = g?.headInfluence ?? 0.6;
    this.gazeBodyInfluence = g?.bodyInfluence ?? 0.3;
    this.gazeSmoothing     = g?.smoothing     ?? 0.15;
    this.gazeCenterX       = g?.centerX      ?? 0.5;
    this.gazeCenterY       = g?.centerY      ?? 0.3;
    if (!this.gazeEnabled) {
      // 無効化時は全値を即座にリセット
      this.gazeTargetX = 0;
      this.gazeTargetY = 0;
      this.gazeEyeCurrentX = 0;
      this.gazeEyeCurrentY = 0;
      this.gazeHeadCurrentX = 0;
      this.gazeHeadCurrentY = 0;
      this.gazeBodyCurrentX = 0;
    }
  }

  /** マウス座標 → gaze ターゲット更新（amadea.app方式: 方向ベクトル正規化） */
  private gazeCenterX = 0.5;
  private gazeCenterY = 0.3;
  setGaze(mouseX: number, mouseY: number, screenW: number, screenH: number): void {
    if (!this.gazeEnabled || !this.model) return;
    const cx = screenW * this.gazeCenterX;
    const cy = screenH * this.gazeCenterY;
    // 中心からの差分
    const dx = (mouseX - cx) / (screenW * 0.5);
    const dy = -(mouseY - cy) / (screenH * 0.5);
    // amadea.app方式: atan2 + cos/sin で方向ベクトルに正規化
    // 距離に関係なく方向だけ見る（常に-1〜1の円上）
    const angle = Math.atan2(dy, dx);
    this.gazeTargetX = Math.cos(angle);
    this.gazeTargetY = Math.sin(angle);
  }

  /** 毎フレーム gaze 補間（シンプルlerp） */
  private updateGaze(delta: number): void {
    const target = this.gazeEnabled
      ? { x: this.gazeTargetX, y: this.gazeTargetY }
      : { x: 0, y: 0 };

    const eyeAlpha = 0.35;
    const headAlpha = 0.35; // 目と同じ速度
    const bodyAlpha = 0.15;
    this.gazeEyeCurrentX += (target.x - this.gazeEyeCurrentX) * eyeAlpha;
    this.gazeEyeCurrentY += (target.y - this.gazeEyeCurrentY) * eyeAlpha;
    // 頭の左右は±0.1に制限（ほんのちょっとだけ動く）
    const headTargetX = target.x; // 制限はapplyGazeToModel側でやる
    this.gazeHeadCurrentX += (headTargetX - this.gazeHeadCurrentX) * headAlpha;
    this.gazeHeadCurrentY += (target.y - this.gazeHeadCurrentY) * headAlpha;
    this.gazeBodyCurrentX += (target.x - this.gazeBodyCurrentX) * bodyAlpha;
  }

  /** coreModel.update()直前にパラメータ反映（amadea.appと同じ係数） */
  private applyGazeToModel(coreModel: any): void {
    if (!this.gazeEnabled) return;
    const ex = this.gazeEyeCurrentX;
    const ey = this.gazeEyeCurrentY;
    const hx = this.gazeHeadCurrentX;
    const hy = this.gazeHeadCurrentY;
    const bx = this.gazeBodyCurrentX;

    // EyeBall: add ×1（focusController.x/yそのまま）
    const idxEyeBallX = this.paramCache.eyeBallX;
    const idxEyeBallY = this.paramCache.eyeBallY;
    if (idxEyeBallX >= 0) coreModel.addParameterValueByIndex(idxEyeBallX, ex, 1.0);
    if (idxEyeBallY >= 0) coreModel.addParameterValueByIndex(idxEyeBallY, ey, 1.0);

    // 頭: 左右控えめ、上下やや多め（amadea.appは×30だが控えめに）
    const idxAngleX = this.paramCache.angleX;
    const idxAngleY = this.paramCache.angleY;
    if (idxAngleX >= 0) {
      coreModel.addParameterValueByIndex(idxAngleX, hx * 0.8, 1.0);
      // AngleXの最終値を±12度に制限
      const currentAngleX = coreModel.getParameterValueByIndex(idxAngleX);
      if (currentAngleX > 5) coreModel.setParameterValueByIndex(idxAngleX, 5, 1.0);
      if (currentAngleX < -5) coreModel.setParameterValueByIndex(idxAngleX, -5, 1.0);
      if (Date.now() % 1000 < 20) console.log(`[GAZE-HEAD] AngleX=${currentAngleX.toFixed(1)} → clamped`);
    }
    if (idxAngleY >= 0) coreModel.addParameterValueByIndex(idxAngleY, hy * 12, 1.0);

    // 体: 左右＋上下
    const idxBodyX = this.paramCache.bodyX;
    const idxBodyY = this.paramCache.bodyY;
    if (idxBodyX >= 0) coreModel.addParameterValueByIndex(idxBodyX, bx * 3, 1.0);
    if (idxBodyY >= 0) coreModel.addParameterValueByIndex(idxBodyY, this.gazeHeadCurrentY * 1.5, 1.0);
  }
