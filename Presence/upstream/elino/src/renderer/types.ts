...
export interface GazeSettings {
  ...
  eyeInfluence: number;   // 0-1, default: 1.0
  headInfluence: number;  // 0-1, default: 0.6
  bodyInfluence: number;  // 0-1, default: 0.3
  smoothing: number;      // 0-1, default: 0.15
  centerX?: number;       // 0-1, 画面幅の割合。default: 0.5（画面中央）
  centerY?: number;       // 0-1, 画面高さの割合。default: 0.3（上寄り）
}
...
export interface CharacterSettings {
  ...
  gaze?: GazeSettings;  // マウス追従（目線・頭・体）
  vrm?: {
...
export interface Settings {
  ...
  tts?: {
    enabled: boolean;
    engine: 'web-speech' | 'voicevox' | 'openai' | 'elevenlabs' | 'google-tts' | 'aivis-speech' | 'style-bert-vits2' | 'openai-compat-tts' | 'fish-audio' | 'kokoro' | 'none';
    ...
    fishAudio?: { apiKey: string; voiceId: string; speed: number; emotion: string };
    kokoro?: { voice: string };
  };
  stt?: {
    enabled: boolean;
...
export interface ElectronAPI {
  ...
  whisperTranscribe: (params: { wavBuffer: ArrayBuffer; model?: string; prompt?: string }) => Promise<string>;
  whisperCheck: () => Promise<{ available: boolean; models: string[] }>;
  // TTS (OpenAI)
  ...
  // Fish Audio S2
  fishAudioSynthesize: (params: { text: string; apiKey: string; voiceId: string; speed: number; emotion: string }) => Promise<ArrayBuffer>;
  fishAudioVoices: (params: { apiKey: string }) => Promise<Array<{ id: string; name: string }>>;
  // Kokoro TTS
  kokoroSynthesize: (params: { text: string; voice: string }) => Promise<ArrayBuffer>;
  // File parsing
