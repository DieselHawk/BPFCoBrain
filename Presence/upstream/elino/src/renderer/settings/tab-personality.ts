...
let personalityCorePersonaTextarea: HTMLTextAreaElement;
let personalityConversationPolicyTextarea: HTMLTextAreaElement;
let personalityVoiceTextarea: HTMLTextAreaElement;
let personalityAvoidancesTextarea: HTMLTextAreaElement;
let personalitySituationalRulesTextarea: HTMLTextAreaElement;
...
export function getEditorPersonality(): Personality {
  const corePersona = personalityCorePersonaTextarea.value.split('\n').filter(l => l.trim());
  const voice = personalityVoiceTextarea.value.split('\n').filter(l => l.trim());
  const examples = personalityExamplesTextarea.value.split('\n').map(s => s.trim()).filter(Boolean);
  return {
    // New structure
    corePersona,
    conversationPolicy: personalityConversationPolicyTextarea.value.split('\n').filter(l => l.trim()),
    voice,
    avoidances: personalityAvoidancesTextarea.value.split('\n').filter(l => l.trim()),
    situationalRules: personalitySituationalRulesTextarea.value.split('\n').filter(l => l.trim()),
    identity: personalityIdentityInput.value.trim() || undefined,
    conversationExamples: examples,
    exampleConversation: examples,
    // Legacy compatibility — selfGrowth still references these
    traits: corePersona,
    speechStyle: voice,
  };
}
...
function loadPersonalityIntoEditor(personality: Personality) {
  ...
  personalityCorePersonaTextarea.value = (isEn && p.corePersonaEn ? p.corePersonaEn : (p.corePersona || p.traits))?.join('\n') || '';
  personalityConversationPolicyTextarea.value = (isEn && p.conversationPolicyEn ? p.conversationPolicyEn : p.conversationPolicy)?.join('\n') || '';
  personalityVoiceTextarea.value = (isEn && p.voiceEn ? p.voiceEn : (p.voice || p.speechStyle))?.join('\n') || '';
  personalityAvoidancesTextarea.value = (isEn && p.avoidancesEn ? p.avoidancesEn : p.avoidances)?.join('\n') || '';
  personalitySituationalRulesTextarea.value = (isEn && p.situationalRulesEn ? p.situationalRulesEn : p.situationalRules)?.join('\n') || '';
  personalityIdentityInput.value = (isEn && p.identityEn) ? p.identityEn : (personality.identity || '');
  personalityExamplesTextarea.value = (isEn && p.conversationExamplesEn ? p.conversationExamplesEn : (p.conversationExamples || p.exampleConversation))?.join('\n') || '';
...
export async function initTab(settings: Settings): Promise<void> {
  ...
  personalityCorePersonaTextarea = document.getElementById('personality-core-persona') as HTMLTextAreaElement;
  personalityConversationPolicyTextarea = document.getElementById('personality-conversation-policy') as HTMLTextAreaElement;
  personalityVoiceTextarea = document.getElementById('personality-voice') as HTMLTextAreaElement;
  personalityAvoidancesTextarea = document.getElementById('personality-avoidances') as HTMLTextAreaElement;
  personalitySituationalRulesTextarea = document.getElementById('personality-situational-rules') as HTMLTextAreaElement;
