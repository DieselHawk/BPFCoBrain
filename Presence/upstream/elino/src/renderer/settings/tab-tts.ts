...
let fishAudioSettingsDiv: HTMLDivElement;
let fishAudioApikeyInput: HTMLInputElement;
let fishAudioVoiceInput: HTMLInputElement;
let fishAudioVoiceSelect: HTMLSelectElement;
let fishAudioVoiceAddIdInput: HTMLInputElement;
let fishAudioVoiceAddNameInput: HTMLInputElement;
let fishAudioVoiceAddBtn: HTMLButtonElement;
let fishAudioVoiceRemoveBtn: HTMLButtonElement;
let fishAudioSpeedInput: HTMLInputElement;
let fishAudioSpeedNumInput: HTMLInputElement;
let fishAudioEmotionSelect: HTMLSelectElement;

// Fish Audio お気に入りボイス
interface FishVoiceFavorite { id: string; name: string; }
const FISH_VOICES_KEY = 'elino-fish-audio-voices';

function loadFishVoices(): FishVoiceFavorite[] {
  try {
...
function saveFishVoices(voices: FishVoiceFavorite[]) {
  localStorage.setItem(FISH_VOICES_KEY, JSON.stringify(voices));
}

function refreshFishVoiceSelect(currentId?: string) {
  const voices = loadFishVoices();
  fishAudioVoiceSelect.innerHTML = '<option value="">デフォルト</option>';
  for (const v of voices) {
    const opt = document.createElement('option');
    opt.value = v.id;
    opt.textContent = v.name ? `${v.name} (${v.id.slice(0, 8)}...)` : v.id;
    fishAudioVoiceSelect.appendChild(opt);
  }
  if (currentId) fishAudioVoiceSelect.value = currentId;
}

let kokoroSettingsDiv: HTMLDivElement;
let kokoroVoiceSelect: HTMLSelectElement;
...
function updateTtsVisibility() {
  ...
  fishAudioSettingsDiv.style.display = engine === 'fish-audio' ? 'block' : 'none';
  kokoroSettingsDiv.style.display = engine === 'kokoro' ? 'block' : 'none';

  // Engine description
  const descEl = document.getElementById('tts-engine-desc');
  if (descEl) {
    const descKeyMap: Record<string, string> = {
      'web-speech': 'settings.tts.engineDesc.webSpeech',
      'voicevox': 'settings.tts.engineDesc.voicevox',
      'openai': 'settings.tts.engineDesc.openai',
      'elevenlabs': 'settings.tts.engineDesc.elevenlabs',
      'google-tts': 'settings.tts.engineDesc.googleTts',
      'aivis-speech': 'settings.tts.engineDesc.aivisSpeech',
      'style-bert-vits2': 'settings.tts.engineDesc.styleBertVits2',
      'openai-compat-tts': 'settings.tts.engineDesc.openaiCompat',
      'fish-audio': 'settings.tts.engineDesc.fishAudio',
      'kokoro': 'settings.tts.engineDesc.kokoro',
      'none': 'settings.tts.engineDesc.none',
    };
    const key = descKeyMap[engine];
    descEl.innerHTML = key ? t(key) : '';
  }
}
...
export async function initTab(settings: Settings): Promise<void> {
  ...
  fishAudioSettingsDiv = document.getElementById('fish-audio-settings') as HTMLDivElement;
  fishAudioApikeyInput = document.getElementById('fish-audio-apikey') as HTMLInputElement;
  fishAudioVoiceInput = document.getElementById('fish-audio-voice') as HTMLInputElement;
  fishAudioVoiceSelect = document.getElementById('fish-audio-voice-select') as HTMLSelectElement;
  fishAudioVoiceAddIdInput = document.getElementById('fish-audio-voice-add-id') as HTMLInputElement;
  fishAudioVoiceAddNameInput = document.getElementById('fish-audio-voice-add-name') as HTMLInputElement;
  fishAudioVoiceAddBtn = document.getElementById('fish-audio-voice-add-btn') as HTMLButtonElement;
  fishAudioVoiceRemoveBtn = document.getElementById('fish-audio-voice-remove-btn') as HTMLButtonElement;
  fishAudioSpeedInput = document.getElementById('fish-audio-speed') as HTMLInputElement;
  fishAudioSpeedNumInput = document.getElementById('fish-audio-speed-num') as HTMLInputElement;
  fishAudioEmotionSelect = document.getElementById('fish-audio-emotion') as HTMLSelectElement;

  // Fish Audio voice favorites
  fishAudioVoiceSelect.addEventListener('change', () => {
    fishAudioVoiceInput.value = fishAudioVoiceSelect.value;
  });

  fishAudioVoiceAddBtn.addEventListener('click', () => {
    const id = fishAudioVoiceAddIdInput.value.trim();
    if (!id) { alert('Voice IDを入力してください'); return; }
    const name = fishAudioVoiceAddNameInput.value.trim();
    const voices = loadFishVoices();
    if (voices.some(v => v.id === id)) { alert('既に追加済みです'); return; }
    voices.push({ id, name: name || '' });
    saveFishVoices(voices);
    refreshFishVoiceSelect(id);
    fishAudioVoiceInput.value = id;
    fishAudioVoiceAddIdInput.value = '';
    fishAudioVoiceAddNameInput.value = '';
  });

  fishAudioVoiceRemoveBtn.addEventListener('click', () => {
    const selected = fishAudioVoiceSelect.value;
    if (!selected) return;
    const voiceName = fishAudioVoiceSelect.options[fishAudioVoiceSelect.selectedIndex]?.textContent || selected;
    if (!confirm(`「${voiceName}」を削除しますか？`)) return;
    const voices = loadFishVoices().filter(v => v.id !== selected);
    saveFishVoices(voices);
    fishAudioVoiceInput.value = '';
    refreshFishVoiceSelect();
  });

  kokoroSettingsDiv = document.getElementById('kokoro-settings') as HTMLDivElement;
  kokoroVoiceSelect = document.getElementById('kokoro-voice') as HTMLSelectElement;
  ...
  fishAudioApikeyInput.value = settings.tts?.fishAudio?.apiKey ?? '';
  fishAudioVoiceInput.value = settings.tts?.fishAudio?.voiceId ?? '';
  refreshFishVoiceSelect(settings.tts?.fishAudio?.voiceId ?? '');
  fishAudioSpeedInput.value = String(settings.tts?.fishAudio?.speed ?? 1.0);
  fishAudioSpeedNumInput.value = String(settings.tts?.fishAudio?.speed ?? 1.0);
  fishAudioEmotionSelect.value = settings.tts?.fishAudio?.emotion ?? 'neutral';

  kokoroVoiceSelect.value = settings.tts?.kokoro?.voice ?? 'af_heart';
  ...
  syncSliders('fish-audio-speed', 'fish-audio-speed-num');
}
...
export function collectSettings(settings: Settings): void {
  settings.tts = {
    ...
    fishAudio: {
      apiKey: fishAudioApikeyInput.value || '',
      voiceId: fishAudioVoiceInput.value || '',
      speed: parseFloat(fishAudioSpeedNumInput.value) || 1.0,
      emotion: fishAudioEmotionSelect.value || 'neutral'
    },
    kokoro: {
      voice: kokoroVoiceSelect.value || 'af_heart'
    }
  };
}
