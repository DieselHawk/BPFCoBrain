...
let gazeEnabledInput: HTMLInputElement;
...
function collectCurrentCharacterSettings(): CharacterSettings {
  return {
    ...
    gaze: {
      enabled: gazeEnabledInput.checked,
      eyeInfluence: 1.0,
      headInfluence: 0.6,
      bodyInfluence: 0.3,
      smoothing: 0.15,
      centerX: parseFloat((document.getElementById('gaze-center-x-input') as HTMLInputElement)?.value) || gazeCenterX,
      centerY: parseFloat((document.getElementById('gaze-center-y-input') as HTMLInputElement)?.value) || gazeCenterY,
    },
...
async function applyCharacterSettingsToForm(char: CharacterSettings) {
  ...
  gazeEnabledInput.checked = char.gaze?.enabled !== false;
  gazeCenterX = char.gaze?.centerX ?? 0.5;
  gazeCenterY = char.gaze?.centerY ?? 0.3;
  updateGazeCenterDisplay();
  const gcxInput = document.getElementById('gaze-center-x-input') as HTMLInputElement;
  const gcyInput = document.getElementById('gaze-center-y-input') as HTMLInputElement;
  if (gcxInput) gcxInput.value = String(gazeCenterX);
  if (gcyInput) gcyInput.value = String(gazeCenterY);
...
// ====== Gaze center display helper ======
function updateGazeCenterDisplay(): void {
  const display = document.getElementById('gaze-center-display');
  if (display) {
    display.textContent = `(${gazeCenterX.toFixed(2)}, ${gazeCenterY.toFixed(2)})`;
  }
}
...
export async function initTab(settings: Settings): Promise<void> {
  ...
  gazeEnabledInput = document.getElementById('gaze-enabled') as HTMLInputElement;
  // 目線中心設定ボタン
  const gazeSetCenterBtn = document.getElementById('gaze-set-center-btn') as HTMLButtonElement;
  if (gazeSetCenterBtn) {
    gazeSetCenterBtn.addEventListener('click', async () => {
      const originalText = gazeSetCenterBtn.textContent || '';
      gazeSetCenterBtn.textContent = t('settings.character.display.gazeCenterWaiting');
      gazeSetCenterBtn.disabled = true;
      try {
        const api = (window as any).electronAPI || (window.parent as any)?.electronAPI;
        const result = await api?.sendGazeCenterRequest?.();
        if (result && typeof result.x === 'number' && typeof result.y === 'number') {
          gazeCenterX = result.x;
          gazeCenterY = result.y;
          updateGazeCenterDisplay();
        }
      } finally {
        gazeSetCenterBtn.textContent = originalText;
        gazeSetCenterBtn.disabled = false;
      }
    });
  }
  // 数値入力でも中心設定できるように
  const gcxNumInput = document.getElementById('gaze-center-x-input') as HTMLInputElement;
  const gcyNumInput = document.getElementById('gaze-center-y-input') as HTMLInputElement;
  if (gcxNumInput) {
    gcxNumInput.addEventListener('change', () => {
      gazeCenterX = parseFloat(gcxNumInput.value) || 0.5;
      updateGazeCenterDisplay();
    });
  }
  if (gcyNumInput) {
    gcyNumInput.addEventListener('change', () => {
      gazeCenterY = parseFloat(gcyNumInput.value) || 0.3;
      updateGazeCenterDisplay();
    });
  }
  ...
  gazeCenterY = char.gaze?.centerY ?? 0.3;
  updateGazeCenterDisplay();
  const gcx2 = document.getElementById('gaze-center-x-input') as HTMLInputElement;
  const gcy2 = document.getElementById('gaze-center-y-input') as HTMLInputElement;
  if (gcx2) gcx2.value = String(gazeCenterX);
  if (gcy2) gcy2.value = String(gazeCenterY);
