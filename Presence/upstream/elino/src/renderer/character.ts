...
document.addEventListener('mousemove', (e) => {
  ...
  // マウス追従（gaze）- Live2Dレンダラーのみ対応
  if (characterSettings?.gaze?.enabled !== false && renderer) {
    const live2d = renderer as any;
    if (typeof live2d.setGaze === 'function') {
      live2d.setGaze(e.clientX, e.clientY, window.innerWidth, window.innerHeight);
    }
  }
});

// --- 目線中心クリック待ちモード ---
let gazeCenterMode = false;

function enterGazeCenterMode(): void {
  gazeCenterMode = true;
  document.body.style.cursor = 'crosshair';
  canvas.style.cursor = 'crosshair';
  // クリックスルーを無効化（クリックを受け取る）
  const api = (window as any).electronAPI;
...
function exitGazeCenterMode(): void {
  gazeCenterMode = false;
...
function setupCanvasListeners(): void {
  canvas.addEventListener('mousedown', (e) => {
    ...
    if (gazeCenterMode) {
      ...
      exitGazeCenterMode();
      const api = (window as any).electronAPI;
      if (api?.sendGazeCenterResult) {
        api.sendGazeCenterResult(x, y);
      }
      return;
    }
...
async function initCharacter() {
  try {
    ...
    // 目線中心クリック待ちモード
    const api = (window as any).electronAPI;
    if (api?.onGazeCenterMode) {
      api.onGazeCenterMode(() => {
        enterGazeCenterMode();
      });
    }

    // グローバルカーソル追跡（ウィンドウ外でもマウス追従）
    if (characterSettings?.gaze?.enabled !== false && api?.startGazePolling) {
      api.startGazePolling();
      api.onGazeCursorUpdate?.((x: number, y: number, w: number, h: number) => {
        const live2d = renderer as any;
        if (typeof live2d?.setGaze === 'function') {
          live2d.setGaze(x, y, w, h);
        }
      });
      console.log('✅ Gaze: グローバルカーソル追跡を開始');
    }
